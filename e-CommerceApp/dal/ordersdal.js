...
Order.createOrder = function (newOrder, result) {
 ...
 con.query("INSERT INTO orders SET ?", newOrder, function (err, res) {
    if (err) {
...
Order.getAllOrder = function (result) {
 ...
 con.query("Select * from Orders", function (err, res) {
    if (err) {
      console.log("error: ", err);
      result(null, err);
    } else {
...
Order.remove = function (id, result) {
 con.query("DELETE FROM orders WHERE order_id = ?", id, function (err, res) {
    if (err) {
      console.log("error: ", err);
      result(null, err);
    } else {
